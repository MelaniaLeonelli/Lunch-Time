	    ...
	    $.get( url , function( data ) {
	  		...
	  		if (data && data.length > 0){
				...
				for (i = 0; i < min(data.length,5); i++){
					let elem= data[i];
					$("#searchResult").append("<div class=\"searchableitem\" data-cod=\""+elem.idprodotto+ "\" onclick=\"openSearch(this)\"><div class=\"item-nme\">"+elem.nome+ "</div><div class=\"item-info\"><span class=\"category\">Categoria: "+elem.categoria+"</span></div></div>");
				}
				$("#searchResult").show();
...
function openSearch(elem){
	...
	window.location.href = 'ProductPageServlet?idcode=' + codice;
}